...
public class VehicleSpawningSystem
{
    ...
    public static void tickAroundPlayer(EntityPlayer player) {
        ...
        List<BaseVehicleEntity> vehiclesAround = world.getEntitiesWithinAABB(BaseVehicleEntity.class, bb, vehicle -> vehicle.hasModuleOfType(AutopilotModule.class));
        if(vehiclesAround.size() < GtwNpcsConfig.vehiclesSpawningRules.getVehicleSpawningLimit()) {
            world.rand.nextInt(1000);
            if(true) {
                PathNode spawnNode = CarPathNodes.getInstance().selectRandomPathNode(player.getPositionVector(), GtwNpcsConfig.vehiclesSpawningRules.getSpawnRadiusMin(),
                        GtwNpcsConfig.vehiclesSpawningRules.getVehicleSpawningRadius(), n -> n.getNodeType() == NodeType.CAR_CITY_LOW_SPED);
                if(spawnNode == null)
                    spawnNode = CarPathNodes.getInstance().selectRandomPathNode(player.getPositionVector(), GtwNpcsConfig.vehiclesSpawningRules.getSpawnRadiusMin(),
                            GtwNpcsConfig.vehiclesSpawningRules.getVehicleSpawningRadius(), n -> n.getNodeType() == NodeType.CAR_CITY);
                if(spawnNode == null)
                    spawnNode = CarPathNodes.getInstance().selectRandomPathNode(player.getPositionVector(), GtwNpcsConfig.vehiclesSpawningRules.getSpawnRadiusMin(),
                            GtwNpcsConfig.vehiclesSpawningRules.getVehicleSpawningRadius(), n -> n.getNodeType() == NodeType.CAR_HIGHWAY);
                if(spawnNode == null)
                    spawnNode = CarPathNodes.getInstance().selectRandomPathNode(player.getPositionVector(), GtwNpcsConfig.vehiclesSpawningRules.getSpawnRadiusMin(),
                            GtwNpcsConfig.vehiclesSpawningRules.getVehicleSpawningRadius(), n -> n.getNodeType() == NodeType.CAR_OFFROAD);
                if(spawnNode == null)
                    return;
                AxisAlignedBB bb1 = new AxisAlignedBB(spawnNode.getPosition().x, spawnNode.getPosition().y, spawnNode.getPosition().z, spawnNode.getPosition().x, spawnNode.getPosition().y, spawnNode.getPosition().z).grow(10);
                if(world.getEntitiesWithinAABB(BaseVehicleEntity.class, bb1).size() > 0)
                    return;
                ...
                GtwNpcMod.network.sendToAllTracking(new SCMessagePutAutopilot(vehicle.getEntityId()), vehicle);
            }
